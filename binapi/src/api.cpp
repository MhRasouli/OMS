...
#define __TRY_BLOCK() \
    try
            ...
            post(bool _signed, const char *target, boost::beast::http::verb action, const std::initializer_list<kv_type> &map, CB cb)
            {
                ...
                if (_signed)
                {
                    ...
                    std::string signature = hmac_sha256(
                        m_sk.c_str(), m_sk.length(), data.c_str(), data.length());
                ...
                else
                {
                    ...
                    async_req_item item{
                        starget, action, std::move(data), std::make_shared<invoker_type>(std::move(cb))};
        ...
        {
            const impl::init_list_type map = {
                {"symbol", symbol}, {"limit", limit}, {"interval", interval}};
        ...
        {
            const impl::init_list_type map = {
                {"symbol", symbol}, {"orderId", orderid}, {"origClientOrderId", client_orderid}};
        ...
        api::result<orders_info_t> api::all_orders(
            const char *symbol, std::size_t orderid, std::size_t start_time, std::size_t end_time, std::size_t limit, all_orders_cb cb)
        {
            const impl::init_list_type map = {
        ...
        api::new_order(
            const char *symbol, const e_side side, const e_type type, const e_time time, const e_trade_resp_type resp, const char *amount, const char *price, const char *client_order_id, const char *stop_price, const char *iceberg_amount, new_order_cb cb)
        {
            ...
            const impl::init_list_type map = {
                {"symbol", symbol}, {"side", side_str}, {"type", type_str}, {"timeInForce", time_str}, {"quantity", amount}, {"price", price}, {"newClientOrderId", client_order_id}, {"stopPrice", stop_price}, {"icebergQty", iceberg_amount}, {"newOrderRespType", responce_type}};
        ...
        api::result<cancel_order_info_t> api::cancel_order(
            const char *symbol, std::size_t order_id, const char *client_order_id, const char *new_client_order_id, cancel_order_cb cb)
        {
            const impl::init_list_type map = {
                {"symbol", symbol}, {"orderId", order_id}, {"origClientOrderId", client_order_id}, {"newClientOrderId", new_client_order_id}};
        ...
        api::result<my_trades_info_t> api::my_trades(
            const char *symbol, std::size_t start_time, std::size_t end_time, std::size_t from_id, std::size_t limit, my_trades_cb cb)
        {
            const impl::init_list_type map = {
