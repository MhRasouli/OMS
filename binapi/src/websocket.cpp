    ...
    cb(__FILE__ "(" BOOST_PP_STRINGIZE(__LINE__) ")", ec.value(), ec.message(), nullptr, 0);
        ...
        struct websocket : std::enable_shared_from_this<websocket>
        {
            ...
            using on_message_received_cb = std::function<
                bool(const char *fl, int ec, std::string errmsg, const char *ptr, std::size_t size)>; // when 'false' returned the stop will called
        ...
        struct websocket::impl
        {
            ...
            void async_connect(boost::asio::ip::tcp::resolver::results_type res, on_message_received_cb cb, holder_type holder)
            {
                ...
                boost::asio::async_connect(
                    m_ws.next_layer().next_layer(), res.begin(), res.end(), [this, cb = std::move(cb), holder = std::move(holder)](boost::system::error_code ec, boost::asio::ip::tcp::resolver::iterator) mutable
                    {
                if ( ec ) {
                    if ( !m_stop_requested ) { __BINAPI_CB_ON_ERROR(cb, ec); }
                } else {
            ...
            void stop()
            {
                ...
                Logger::logger.log_special(LogLocation, false, "stop()->before check is_open()");
                if (m_ws.next_layer().next_layer().is_open())
                {
                    Logger::logger.log_special(LogLocation, false, "stop()->before close()");
                    boost::system::error_code ec;
                    m_ws.close(boost::beast::websocket::close_code::normal, ec);
                    Logger::logger.log_special(LogLocation, false, "stop()->after close normal(), ec=%i", ec.value());

                    if (ec)
                    {
                        m_ws.close(boost::beast::websocket::close_code::abnormal, ec);
                        Logger::logger.log_special(LogLocation, false, "stop()->after close abnormal(), ec=%i", ec.value());
                    }
                }
        ...
        struct websockets::impl
        {
            impl(boost::asio::io_context &ioctx, std::string host, std::string port, on_message_received_cb cb)
                : m_ioctx{ioctx}, m_host{std::move(host)}, m_port{std::move(port)}, m_on_message{std::move(cb)}, m_map{}
            {
            }
            ...
            {
                return stop_channel_impl(h, [](const auto &sp)
                                         { sp->stop(); });
            }
            ...
            {
                return unsubscribe_all_impl([](const auto &sp)
                                            { sp->stop(); });
            }
